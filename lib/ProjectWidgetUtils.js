...
const ProjectWidgetUtils = function () {
  ...
  this.homePageWaiting = function () {
    ...
    if (this.widgetCheck(_config.stroll_end_ui_content || '返回我的森林', 1000)) {
      errorInfo('错误位置：当前所在位置为逛一逛结束界面')
      return false
    }
    if (this.widgetCheck(_config.friend_home_check_regex || 'TA收取你', 1000)) {
    ...
    //   return false
    // }
    return this.widgetWaiting(_config.home_ui_content, '个人首页', 1000)
  }
  ...
  this.getFriendCurrentEnergy = function () {
    ...
    if (friendGet) {
      try {
        let container = friendGet.parent().child(0).children().findOne(textMatches(/\d+g/))
        if (container) {
  ...
  this.wateringFriends = function () {
    ...
    if (wateringWidget) {
      automator.clickPointRandom(wateringWidget.centerX, wateringWidget.centerY)
      ...
      if (this.widgetCheck(give_content, 5000) && (giveHimButton = this.widgetGetOne(give_content))) {
        ...
        sleep(400)
        automator.clickPointRandom(parseInt(bounds.left + bounds.width() / 2), parseInt(bounds.top + bounds.height() / 2))
        sleep(500)
        return true
      } else {
  ...
  this.wateringCooperationPlant = function () {
    ...
    if (pageWidget) {
      ...
      if (currentPage < totalPage) {
        ...
        if (nextButton) {
          automator.clickRandom(nextButton)
          sleep(1500)
          this.wateringCooperationPlant()
        }
      } else {
  ...
  this.doWateringToCooperationPlant = function () {
    ...
    if (point) {
      ...
      if (editWidget) {
        ...
        if (confirmButton) {
          automator.clickRandom(confirmButton)
          sleep(1000)
        }
  ...
  this.checkIsDuplicateCardUsed = function () {
    let target = null
    if ((target = this.widgetGetOne(/^\d{2}:\d{2}|\d+天$/, 1000, true, null, matcher => matcher.boundsInside(0, _config.device_height * 0.3, _config.device_width / 2, _config.device_height * 0.7))) != null) {
